...
fn generateTestKeys(allocator: Allocator, count: usize) ![][]const u8 {
    ...
    const words = [_][]const u8{ "authentication", "memory", "optimization", "algorithm", "database", "search", "index", "cache", "performance", "security", "parsing", "validation", "transformation", "compression", "encryption" };
...
fn generateTestValues(allocator: Allocator, count: usize) ![][]const u8 {
    ...
    for (values, 0..) |*value, i| {
        ...
        if (size_category < 0.4) {
            ...
            value.* = try std.fmt.allocPrint(allocator, "This is a medium-sized content block that represents typical data stored in Agrama. " ++
                "It contains structured information about algorithms, performance characteristics, " ++
                "and implementation details. Item {d} has complexity O(n) and memory usage {d}MB.", .{ i, rng.random().intRangeAtMost(u32, 1, 100) });
        } else {
            // Large values (1000+ bytes) - 20% of data
            value.* = try std.fmt.allocPrint(allocator, "This is a large content block representing substantial documentation, code, or analysis results. " ++
                ...
                "and sub-10ms for complex hybrid queries combining semantic, lexical, and graph-based search modalities.", .{i});
        }
    }
...
fn benchmarkSearchPrimitive(allocator: Allocator, config: BenchmarkConfig) !BenchmarkResult {
    ...
    const queries = [_][]const u8{ "authentication", "memory optimization", "algorithm performance", "database search", "index cache", "security validation", "parsing transformation", "compression encryption" };
...
fn benchmarkLinkPrimitive(allocator: Allocator, config: BenchmarkConfig) !BenchmarkResult {
    ...
    const relations = [_][]const u8{ "depends_on", "evolved_into", "similar_to", "contains", "references", "implements", "extends", "calls", "inherits_from", "composed_of" };
...
fn benchmarkTransformPrimitive(allocator: Allocator, config: BenchmarkConfig) !BenchmarkResult {
    ...
    const operations = [_][]const u8{ "parse_functions", "extract_imports", "generate_summary", "compress_text", "diff_content", "analyze_complexity" };
...
fn benchmarkConcurrentPrimitives(allocator: Allocator, config: BenchmarkConfig) !BenchmarkResult {
    ...
    const agents = [_][]const u8{ "agent_1", "agent_2", "agent_3", "agent_4", "agent_5", "agent_6", "agent_7", "agent_8", "agent_9", "agent_10" };
