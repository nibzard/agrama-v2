        ...
        while (iterator.next()) |entry| {
            ...
            if (measurements.len > 0) {
                ...
                try file.writer().print("{s},{d},{d:.3},{d},{d:.1}\n", 
                                      .{ name, baseline.latency_ns, baseline.throughput_ops_per_sec, 
                                        baseline.memory_bytes, baseline.cpu_percent });
            }
        }
    ...
    pub fn print_summary(self: RegressionResult) void {
        ...
        if (self.baseline_measurement) |baseline| {
            ...
            print("  Memory: {:.1} → {:.1} KB ({:.1}% change)\n", 
                  .{ @as(f64, @floatFromInt(baseline.memory_bytes)) / 1024.0, 
                     @as(f64, @floatFromInt(self.current_measurement.memory_bytes)) / 1024.0, 
...
pub const PerformanceRegressionDetector = struct {
    ...
    pub fn detectRegression(self: *PerformanceRegressionDetector, current: PerformanceMeasurement) !RegressionResult {
        ...
        if (result.baseline_measurement) |baseline| {
            ...
            result.latency_change_percent = calculatePercentageChange(
                @as(f64, @floatFromInt(baseline.latency_ns)),
                @as(f64, @floatFromInt(current.latency_ns))
            );
            
            result.throughput_change_percent = calculatePercentageChange(
                baseline.throughput_ops_per_sec,
                current.throughput_ops_per_sec
            );
            
            result.memory_change_percent = calculatePercentageChange(
                @as(f64, @floatFromInt(baseline.memory_bytes)),
                @as(f64, @floatFromInt(current.memory_bytes))
            );
    ...
    pub fn benchmarkWithRegressionDetection(
        self: *PerformanceRegressionDetector,
        name: []const u8,
