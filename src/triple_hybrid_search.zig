...
pub const HybridQuery = struct {
    ...
    pub fn adjustWeightsForQueryType(self: *HybridQuery, query_type: QueryType) void {
        switch (query_type) {
            .exact_keyword => {
                self.alpha = 0.7; self.beta = 0.2; self.gamma = 0.1;
            },
            .semantic_concept => {
                self.alpha = 0.2; self.beta = 0.7; self.gamma = 0.1;
            },
            .dependency_related => {
                self.alpha = 0.1; self.beta = 0.2; self.gamma = 0.7;
            },
            .balanced => {
                self.alpha = 0.4; self.beta = 0.4; self.gamma = 0.2;
            },
        }
    }
