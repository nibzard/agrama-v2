    ...
    fn checkAlerts(self: *PrimitivePerformanceMonitor) !void {
        ...
        while (tracker_iter.next()) |entry| {
            ...
            if (p99_latency > self.alert_thresholds.max_latency_p99_ms) {
                const message = try std.fmt.allocPrint(self.allocator, "High P99 latency detected for {s}: {d:.2}ms (threshold: {d:.2}ms)", .{ entry.key_ptr.*, p99_latency, self.alert_thresholds.max_latency_p99_ms });
        ...
        if (current_throughput < self.alert_thresholds.min_throughput_qps) {
            const message = try std.fmt.allocPrint(self.allocator, "Low throughput detected: {d:.2} QPS (threshold: {d:.2} QPS)", .{ current_throughput, self.alert_thresholds.min_throughput_qps });
        ...
        if (current_memory_mb > self.alert_thresholds.max_memory_mb) {
            const message = try std.fmt.allocPrint(self.allocator, "High memory usage detected: {d:.2}MB (threshold: {d:.2}MB)", .{ current_memory_mb, self.alert_thresholds.max_memory_mb });
