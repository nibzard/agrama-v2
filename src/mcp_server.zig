...
pub const MCPServer = struct {
    ...
    pub fn initWithAdvancedFeatures(allocator: Allocator, database: *Database) !MCPServer {
    ...
    pub fn deinit(self: *MCPServer) void {
        ...
        if (self.hybrid_search) |search| {
            search.deinit();
            self.allocator.destroy(search);
        }

        if (self.fre_engine) |fre| {
            fre.deinit();
            self.allocator.destroy(fre);
        }

        if (self.semantic_db) |semantic| {
            semantic.deinit();
            self.allocator.destroy(semantic);
        }
    ...
    fn processMessage(self: *MCPServer, message: []const u8) !void {
    ...
    fn parseRequest(self: *MCPServer, value: std.json.Value) !MCPRequest {
    ...
    /// Handle MCP request
    fn handleRequest(self: *MCPServer, request: MCPRequest) !void {
        defer {
    ...
    fn handleInitialize(self: *MCPServer, request: MCPRequest) !void {
    ...
    fn handleInitialized(self: *MCPServer, request: MCPRequest) !void {
        _ = request;
    ...
    fn handleToolsList(self: *MCPServer, request: MCPRequest) !void {
        // Use arena allocator for temporary JSON structures to avoid leaks
        var arena = std.heap.ArenaAllocator.init(self.allocator);
        defer arena.deinit();
        const json_allocator = arena.allocator();
    ...
    fn handleToolsCall(self: *MCPServer, request: MCPRequest) !void {
    ...
    fn callTool(self: *MCPServer, tool_name: []const u8, arguments: std.json.Value) !MCPToolResponse {
    ...
    fn callToolInternal(self: *MCPServer, tool_name: []const u8, arguments: std.json.Value) !MCPToolResponse {
        // Core enhanced tools
    ...
    fn executeReadCode(self: *MCPServer, arguments: std.json.Value) !MCPToolResponse {
    ...
    fn findSimilarFiles(self: *MCPServer, path: []const u8, max_results: u32) ![]@import("semantic_database.zig").SemanticSearchResult {
    ...
    fn executeWriteCode(self: *MCPServer, arguments: std.json.Value) !MCPToolResponse {
        const args_obj = arguments.object;
        const path_value = args_obj.get("path") orelse return error.MissingPath;
        const path = path_value.string;
    ...
    fn registerAgentActivity(self: *MCPServer, agent_id: []const u8, agent_name: []const u8) !void {
    ...
    fn generateMockEmbedding(self: *MCPServer, content: []const u8) !hnsw.MatryoshkaEmbedding {
    ...
    fn executeGetContext(self: *MCPServer, arguments: std.json.Value) !MCPToolResponse {
        const args_obj = arguments.object;
    ...
    fn executeSemanticSearch(self: *MCPServer, arguments: std.json.Value) !MCPToolResponse {
    ...
    fn executeAnalyzeDependencies(self: *MCPServer, arguments: std.json.Value) !MCPToolResponse {
    ...
    fn executeHybridSearch(self: *MCPServer, arguments: std.json.Value) !MCPToolResponse {
    ...
    fn executeRecordDecision(self: *MCPServer, arguments: std.json.Value) !MCPToolResponse {
        const args_obj = arguments.object;
    ...
    fn executeQueryHistory(self: *MCPServer, arguments: std.json.Value) !MCPToolResponse {
        const args_obj = arguments.object;
        const path = if (args_obj.get("path")) |val| val.string else null;
    ...
    fn createErrorResponse(self: *MCPServer, error_message: []const u8) !MCPToolResponse {
    ...
    fn createTextResponse(self: *MCPServer, parts: [][]const u8) !MCPToolResponse {
        var total_length: usize = 0;
        for (parts) |part| {
            total_length += part.len;
        }
    ...
    fn sendResponse(self: *MCPServer, id: ?std.json.Value, result: std.json.Value) !void {
    ...
    /// Send JSON-RPC error
    fn sendError(self: *MCPServer, id: ?std.json.Value, code: i32, message: []const u8, data: ?std.json.Value) !void {
    ...
    fn sendMessage(self: *MCPServer, response: MCPResponse) !void {
...
test "MCPServer initialization" {
    ...
    var server = try MCPServer.init(allocator, &db);
    defer server.deinit();

    try testing.expect(server.tools.items.len == 8);
